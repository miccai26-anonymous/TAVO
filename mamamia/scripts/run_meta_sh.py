#!/usr/bin/env python3
"""
Successive Halving CMA-ES meta-optimization using nnUNet proxy fitness (PlainConvUNet).

Usage:
    python run_meta_sh.py \
        ...
        --output-dir ./outputs/meta/sh_eta3_nnunet500
"""
...
from meta.optimizers.hyperband import SuccessiveHalvingCMAES
...
def main():
    ...
    print("=" * 60)
    print(f"Successive Halving CMA-ES (nnUNet Proxy, PlainConvUNet)")
    print("=" * 60)
    ...
    optimizer = SuccessiveHalvingCMAES(methods=methods, seed=args.seed)
    optimizer.initialize({
        "R": args.R, "eta": args.eta,
        "n_candidates": args.n_candidates, "sigma0": 0.3,
    })
    ...
    def save_checkpoint(gen, best_w, best_f, all_hist, gen_sums, tot_steps):
        ...
        checkpoint = {
            "config": {
                "optimizer": "successive_halving_cmaes", "proxy_model": "PlainConvUNet_nnunet2d",
    ...
    if checkpoint_path.exists() and history_path.exists():
        ...
        # Replay completed generations to restore CMA-ES state
        max_stage = len(optimizer.schedule) - 1
        for g in range(completed_gens):
            gen_hist = [h for h in saved_history if h.get("generation") == g + 1]

            solutions = optimizer.es.ask()  # advance CMA-ES internal state

            # Reconstruct per-candidate final results (highest stage reached)
            candidate_final = {}
            for h in gen_hist:
                idx = h["candidate_idx"]
                if idx not in candidate_final or h["stage"] > candidate_final[idx]["stage"]:
                    candidate_final[idx] = h

            # Compute adjusted fitnesses (same formula as run_generation)
            cmaes_fitnesses = []
            for i in range(len(solutions)):
                if i in candidate_final:
                    f = candidate_final[i]["fitness"]
                    s = candidate_final[i]["stage"]
                    adjusted = f - optimizer.stage_penalty * (max_stage - s)
                else:
                    adjusted = float("-inf")
                cmaes_fitnesses.append(-adjusted)

            optimizer.es.tell(solutions, cmaes_fitnesses)

        start_gen = completed_gens
        print(f"  Resumed: gen {completed_gens}, best={global_best_fitness:.4f}")
    ...
    for gen in range(start_gen, args.generations):
        ...
        gen_summary = {
            ...
            "candidates_per_stage": stage_counts,
        ...
        print(f"  Steps: {gen_steps}, Cumulative: {total_steps}")
    ...
    with open(args.output_dir / f"selection_{args.budget}_{tag}.json", "w") as f:
        json.dump({
            "selected": selected,
            "selected_scores": {c: float(combined[c]) for c in selected},
            "method": f"sh_nnunet_proxy_{args.fitness}_eta{args.eta}",
