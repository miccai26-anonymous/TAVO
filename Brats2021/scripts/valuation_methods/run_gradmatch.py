...
def main():
    ...
    parser.add_argument("--T", type=int, required=True,
                        help="Target train size (UPENN=100, IVYGAP=10)")
    parser.add_argument("--normalize", action="store_true")
    parser.add_argument("--max_rank", type=int, default=750)
    args = parser.parse_args()

    base_root = "/path/to/project"

    # 👇 GradMatch uses ORIENT embeddings (same case-level gradient vectors)
    ...
    out_root = os.path.join(
        base_root,
        "data",
        f"splits_{args.target}_gradmatch"
    )
    ...
    # 1️⃣ FULL RANKING
    # ============================================================
    selected_order, gains = gradmatch_full_ranking(
        src_vecs,
        tgt_vecs,
    ...
    # ============================================================
    # 2️⃣ Build score dict (ranking-based)
    # ============================================================
    ...
    np.save(
        os.path.join(out_root, "gradmatch_score_dict.npy"),
        score_dict,
        allow_pickle=True
    )
