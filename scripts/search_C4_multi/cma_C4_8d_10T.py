...
def get_template_yaml(repeat_id: int):
    return (
        PROJECT_ROOT
        / "configs_C4_cma"
...
SCORE_ROOT = PROJECT_ROOT / "data/splits_C4_mix_scores_multi"
...
WARMUP_CKPT_MAP = {
    f"repeat{i:02d}":
    f"/path/to/workspace/"
    f"ANON_USER/EfficientVit/outputs_C4_new/source_plus_target_repeat{i:02d}/epoch_007.pt"
    for i in range(1, 6)
}
    ...
    *,
    ...
    for sid in selected_subjects:
        if sid in subject_to_slice_indices:
            ...
            missing += 1
    if missing > 0:
        print(f"⚠️ Warning: {missing} selected subjects not found in full_source_dataset map")
...
def stageA2_cma(
    ...
    template_yaml = get_template_yaml(repeat_id)
    ...
    full_source_dataset = BraTSSliceDataset(
        ...
        split="train",
        img_size=img_size,
        split_txt_dir=source_dom.get("split_txt"),
    ...
    score_dicts = load_norm_scores(SCORE_ROOT, repeat_id=repeat_id)
    ...
    gens = []
    # ---------- partial save ----------
    save_dir = OUT_ROOT / f"repeat{repeat_id:02d}"/ f"{budget_T}T" / run_tag
    save_dir.mkdir(parents=True, exist_ok=True)
    partial_json_path = save_dir / "stageA2_cma_partial.json"

    print("\n==============================")
    print("🚀 CMA-ES Stage A2")
    ...
    for c in corners:
        r = run_candidate(
            repeat_id, score_dicts, budget_T, run_tag, c, it_eval, seeds_eval,
    ...
    for g in range(n_gen):
        ...
        for k in range(lam):
            z = arx[k]
            r = run_candidate(
                repeat_id, score_dicts, budget_T, run_tag, z, it_eval, seeds_eval,
                template_cfg=template_cfg,
                full_source_dataset=full_source_dataset,
                target_dataset=target_dataset,
                val_dataset=val_dataset,
    ...
    for r in all_sorted[:A2_CFG["refine_topk"]]:
        ...
        rr = run_candidate(
            repeat_id, score_dicts, budget_T, run_tag, vec, A2_CFG["iters_refine"], seeds_refine,
            template_cfg=template_cfg,
            full_source_dataset=full_source_dataset,
            target_dataset=target_dataset,
            val_dataset=val_dataset,
